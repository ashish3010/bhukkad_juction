...
export type CheckoutAddressFlowProps = {
  /**
   * After cancelling the address form with no entries left in the book:
   * `navigate-order-summary` — used on `/checkout/delivery` (same as previous DeliveryScreen).
   * `stay-on-form` — used when embedded on `/order-summary` desktop so we stay on the page.
   */
  emptyBookCancel: "navigate-order-summary" | "stay-on-form";
  /** Forwarded to `DesktopFinalizeCheckoutView` on wide screens. */
  hidePaymentAndCouponOnDesktop?: boolean;
  /** Cart edit link on desktop finalize (default `/order-summary`). */
  editCartHref?: string;
};

export function CheckoutAddressFlow({
  emptyBookCancel,
  hidePaymentAndCouponOnDesktop = false,
  editCartHref,
}: CheckoutAddressFlowProps) {
  const router = useRouter();
  const { lines } = useCart();
  const [phase, setPhase] = useState<Phase>("checking");
  const [book, setBook] = useState<SavedAddressEntry[]>([]);
  const [editingEntry, setEditingEntry] = useState<SavedAddressEntry | null>(
    null,
  );
  ...
  const onFormCancel = useCallback(() => {
    ...
    if (entries.length > 0) {
      ...
      setEditingEntry(null);
      setPhase("list");
    } else if (emptyBookCancel === "navigate-order-summary") {
  ...
  }, [emptyBookCancel, refreshBook, router]);
  ...
  const onFormSaved = useCallback(
    async (orderNote?: string) => {
      refreshBook();
      setEditingEntry(null);
      const snap = writeOrderPlacedSnapshot(lines, { orderNote });
      if (!snap) return;
      try {
        await fetch("/api/notify-order", {
          method: "POST",
  ...
  return (
    <DeliveryAddressForm
      ...
      onSaved={onFormSaved}
      hidePaymentAndCouponOnDesktop={hidePaymentAndCouponOnDesktop}
